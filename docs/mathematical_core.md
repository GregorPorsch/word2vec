...
Negative sampling (Mikolov et al., 2013) replaces the expensive softmax with a binary classification objective. Instead of computing the partition function over $V$ words, we:
...
For a positive pair $(w, c)$ and $K$ negative samples $\{k_1, \ldots, k_K\}$ drawn from the noise distribution $P_n$:

$$
\mathcal{L}(w, c) = -\log \sigma(\mathbf{u}_c^\top \mathbf{v}_w) - \sum_{i=1}^{K} \log \sigma(-\mathbf{u}_{k_i}^\top \mathbf{v}_w)
$$
...
This avoids computing $e^{|x|}$ for large $|x|$, which would overflow. The two forms are algebraically identical but numerically distinct.
...
*Derivation:* Let $z = \mathbf{u}_c^\top \mathbf{v}_w$. Then $\frac{\partial z}{\partial \mathbf{v}_w} = \mathbf{u}_c$ and $\frac{d}{dz}[-\log \sigma(z)] = -(1 - \sigma(z))$.
...
*Derivation:* Let $z_i = \mathbf{u}_{k_i}^\top \mathbf{v}_w$. Then $\frac{\partial(-z_i)}{\partial \mathbf{v}_w} = -\mathbf{u}_{k_i}$ and $\frac{d}{dz}[-\log \sigma(-z)] = \sigma(z)$.
...
Let $z = \mathbf{u}_c^\top \mathbf{v}_w$. Then $\frac{\partial z}{\partial \mathbf{u}_c} = \mathbf{v}_w$.
...
Let $z_i = \mathbf{u}_{k_i}^\top \mathbf{v}_w$. Then $\frac{\partial z_i}{\partial \mathbf{u}_{k_i}} = \mathbf{v}_w$ and $\frac{d}{dz}[-\log\sigma(-z)] = \sigma(z)$.
...
**Total parameters updated per step:** $1 + 1 + K$ rows of the embedding matrices.
