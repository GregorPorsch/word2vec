...
- v_w = W_in[w]  : input embedding for center word w       (D,)
- u_c = W_out[c] : output embedding for context word c     (D,)
- u_k = W_out[k] : output embedding for negative sample k  (D,)

Objective (negative sampling)
-----------------------------
For a positive pair (w, c) and K negative samples {k_1 … k_K}::

    L = −log σ(u_c · v_w) − Σ_k log σ(−u_k · v_w)

Gradients
---------
Let σ_c = σ(u_c · v_w) and σ_k = σ(u_k · v_w).

∂L/∂v_w  = −(1 − σ_c) u_c  + Σ_k σ_k u_k
∂L/∂u_c  = −(1 − σ_c) v_w
∂L/∂u_k  =  σ_k v_w              (for each negative sample k)
...
class SkipGramModel:
    ...
    def train_step(
        ...
        applies the SGD parameter update — all in one call to avoid
        ...
        """
        # ---- Embedding look-up ------------------------------------------
        v_w = self.W_in[center_id].copy()    # (D,)  — copy because we update in-place
        ...
        # ---- Gradient computation (see docstring for derivation) --------
        ...
        #   σ_k = σ(u_k · v_w)
        #   ∂L/∂u_k = σ_k v_w
        #   contribution to ∂L/∂v_w: Σ_k σ_k u_k
